...
tags: [csharp, dotnet, aspnet-core, java-to-csharp, learning]
---

# Java 開發者轉向現代 C# / .NET / ASP.NET Core

這是一份給「已經會 Java、也碰過 .NET / Web API，但對現代 C# 寫法還不熟」的系統化教材。

學習路線不是從變數、`if`、`for` 開始，而是：

> Java 已經會 → 對照 Java → 補 C# 特有語法 → 理解 .NET 慣用方式 → 放進 ASP.NET Core 實際情境
...
1. [[01-型別與值語意]]：先建立 C# 與 Java 最容易錯位的型別模型。
...
7. [[13-ASP.NET-Core架構]] → [[14-WebApplicationFactory]]。
8. [[15-綜合實作]]，最後用 [[16-面試快速複習]] 和 [[17-學習分級與路線圖]] 收斂。
9. MSSQL 基礎：[[18-SQLServer資料型別]] → [[19-SQLServer-NULL與三值邏輯]] → [[20-SQLServer-JOIN]] → [[21-SQLServer-Subquery-CTE-View]] → [[22-SQLServer-Window-Functions]]。
10. SQL 效能與併發：[[23-SQLServer-Index]] → [[24-Execution-Plan與Query-Performance]] → [[25-SQLServer-Transaction與Isolation]] → [[26-SQLServer-Lock-Blocking-Deadlock]]。
11. SQL Server + EF Core：[[27-SQLServer-Programmability-Temp-Pagination]] → [[28-EF-Core-Query與Methods]] → [[29-EF-Core-Related-Data-Tracking-N+1]] → [[30-EF-Core-Concurrency與Migration]] → [[31-User-Order整合實作]]。
12. 最後用 [[32-MSSQL面試快速複習]] 和 [[33-MSSQL學習優先級]] 收斂。
...
| 04 | [[04-LINQ]] | 把 Java Stream 思維轉成 C# LINQ |
...
| 09 | [[09-DependencyInjection]] | 以 Spring DI 經驗理解 ASP.NET Core DI lifetime |
...
| 13 | [[13-ASP.NET-Core架構]] | 把 middleware、endpoint、service、DB 串成一條 request pipeline |
...
每章中的 code block 優先追求「短、可以放進真實 Web API、能直接解釋設計意圖」。複製範例時，請先用 `dotnet new webapi` 建立專案，再依章節加入相應程式碼；完整範例的建議目錄與檔案拆分見 [[15-綜合實作]]。
